...
app.set("PORT", process.env.PORT || 3000);	// PORT에 값이 지정되어있지 않으면 기본값으로 3000

app.use("/", express.static(path.join(__dirname, 'public')));	// 절대 경로
...
app.get("/", (req, res, next) => {
	...
	*/
	...
	next(err);	// -> error 객체가 인수로 있으면 -> 에러처리 미들웨어로 이동
...
app.use((req, res, next) => {
	...
	err.status = 404;
	next(err);	// 오류처리 미들웨어로 전달
});

// 오류 처리 라우터 -> 반드시 4개의 인수가 유지되어야한다 (첫번째 인수가 에러 객체)
app.use((err, req, res, next) => {

	return res.status(err.status || 500).send(err.message);	// err.status가 있으면 출력, 없으면 기본값으로 500을 출력한다
});
